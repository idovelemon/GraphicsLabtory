...
void DeviceImp::Initialize() {
    ...
    log::LogPrint("OpenGL Version: %s", reinterpret_cast<LPCSTR>(version));

    const GLubyte* vendor = glGetString(GL_VENDOR);
    log::LogPrint("OpenGL Vendor: %s", reinterpret_cast<LPCSTR>(vendor));

    const GLubyte* render = glGetString(GL_RENDERER);
    log::LogPrint("OpenGL Render: %s", reinterpret_cast<LPCSTR>(render));

    const GLubyte* glew_version = glewGetString(GLEW_VERSION);
    log::LogPrint("GLEW Version: %s", reinterpret_cast<LPCSTR>(glew_version));
}
