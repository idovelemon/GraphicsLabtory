...
void main() {
	vec3 hdr_color = texture2D(glb_HDRSceneTex, vs_texcoord).xyz;
    ...
    // base tone mapping
    hdr_color = hdr_color / (hdr_color + vec3(1.0, 1.0, 1.0));

    // gamma correction
    hdr_color = pow(hdr_color, vec3(1.0 / 2.2, 1.0 / 2.2, 1.0 / 2.2));
