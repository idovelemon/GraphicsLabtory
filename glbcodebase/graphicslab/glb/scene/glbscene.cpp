...
public:
    ...
    int32_t AddDecalObject(const char* meshFile, const char* materialFile);
...
int32_t SceneImp::AddObject(const char* meshFile, const char* materialGroupFile) {
    int32_t id = -1;

    if (meshFile != nullptr) {
        Object* obj = Object::Create(meshFile, materialGroupFile);
        if (obj != nullptr) {
            id = FindEmptyID();
            if (id != -1) {
                m_ObjectDataBase[id] = obj;
...
int32_t SceneImp::AddDecalObject(const char* meshFile, const char* materialFile) {
    int32_t id = -1;

    if (meshFile != nullptr && materialFile != nullptr) {
        Object* obj = DecalObject::Create(meshFile, materialFile);
        if (obj != nullptr) {
            id = FindEmptyID();
            if (id != -1) {
                m_ObjectDataBase[id] = obj;
...
int32_t Scene::AddObject(const char* meshFile, const char* materialFile) {
    int32_t result = -1;

    if (s_SceneImp != nullptr) {
        result = s_SceneImp->AddObject(meshFile, materialFile);
    } else {
        GLB_SAFE_ASSERT(false);
    }
...
int32_t Scene::AddDecalObject(const char* meshFile, const char* materialFile) {
    int32_t result = -1;

    if (s_SceneImp != nullptr) {
        result = s_SceneImp->AddDecalObject(meshFile, materialFile);
    } else {
        GLB_SAFE_ASSERT(false);
    }
