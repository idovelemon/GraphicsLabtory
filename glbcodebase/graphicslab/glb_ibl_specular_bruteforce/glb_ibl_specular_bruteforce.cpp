...
// Date: 2018 / 02 / 01
// Brief: Demostrate how to implement specular image based lighting
//----------------------------------------------------------------------
#include "glb_ibl_specular_bruteforce.h"
#include "resource.h"
...
public:
    bool Initialize() {
        ...
        m_PBSProgram_CubeMapLoc = m_PBSProgram->GetUniformLocation("glb_CubeMap");
    ...
    void DrawSphere() {
        ...
        render::Device::ClearTexture();
        render::Device::SetTexture(0, m_DiffuseCubeMap, 0);
        render::Device::SetTexture(1, m_CubeMap, 1);
        ...
        for (int32_t i = 0; i < 6; i++) {
            for (int32_t j = 0; j < 6; j++) {
                ...
                render::Device::SetUniformSamplerCube(m_PBSProgram_CubeMapLoc, 1);
...
protected:
    ...
    int32_t                         m_PBSProgram_CubeMapLoc;
...
int _stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPSTR cmdLine, int nShowCmd) {
    ...
    memcpy(config.caption, L"glb_ibl_specular", sizeof(L"glb_ibl_specular"));
