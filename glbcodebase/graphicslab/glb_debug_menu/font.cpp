...
static class FontMgrImp* s_FontMgrImp = nullptr;
...
void FontMgrImp::GetCharacter(const char c, float& ltU, float& ltV, float& rbU, float& rbV, float& ndcSizeX, float& ndcSizeY) {
    glb::render::texture::Texture* fontTex = glb::render::texture::Mgr::GetTextureById(m_FontTexture);
    int32_t width = fontTex->GetWidth();
    int32_t height = fontTex->GetHeight();
    int32_t screenWidth = glb::app::Application::GetWindowWidth();
    int32_t screenHeight = glb::app::Application::GetWindowHeight();
...
void FontMgrImp::GetTextSize(const char* text, float& ndcSizeX, float& ndcSizeY) {
...
void FontMgr::GetTextSize(const char* text, float& ndcSizeX, float& ndcSizeY) {
    if (s_FontMgrImp != nullptr) {
        s_FontMgrImp->GetTextSize(text, ndcSizeX, ndcSizeY);
    } else {
        GLB_SAFE_ASSERT(false);
    }
}
