...
float calculateShadowFactor(vec3 pos, vec3 eyePos, vec3 lookAt, mat4 shadowM, sampler2D shadowMap) {
	float shadowFactor = 1.0;
	...
	lightSpacePos.xyz /= 2.0f;
	lightSpacePos.xyz += 0.5f;
	shadowFactor = texture2D(shadowMap, lightSpacePos.xy).z;
...
void main() {
	...
	float shadow_factor = calculateShadowFactor(vs_Vertex.xyz, glb_unif_EyePos, glb_unif_LookAt, glb_unif_ShadowM, glb_unif_ShadowMap);
