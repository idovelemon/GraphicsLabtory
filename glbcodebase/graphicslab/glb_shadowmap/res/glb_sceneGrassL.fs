...
void glbPixelDepthOffset(float offset) {
	gl_FragDepth = gl_FragCoord.z + offset;
}

float glbDithering(float ditherValue, vec2 screenPos) {
    mat4 dithering_matrix;
    dithering_matrix[0] = vec4(1.0 / 17.0, 9.0 / 17.0, 3.0 / 17.0, 11.0 / 17.0);
    dithering_matrix[1] = vec4(13.0 / 17.0, 5.0 / 17.0, 15.0 / 17.0, 7.0 / 17.0);
    dithering_matrix[2] = vec4(4.0 / 17.0, 12.0 / 17.0, 2.0 / 17.0, 10.0 / 17.0);
    dithering_matrix[3] = vec4(16.0 / 17.0, 8.0 / 17.0, 14.0 / 17.0, 6.0 / 17.0);

    if (ditherValue < dithering_matrix[int(mod(int(screenPos.x), 4))][int(mod(int(screenPos.y), 4))]) return 1.0;	
	else return 0.0;
}
...
void main() {
	...
	normalInWorld = vec3(0.0, 1.0, 0.0);
	...
	vec3 albedo = glb_unif_Albedo * mask.xyz * mask.w;
