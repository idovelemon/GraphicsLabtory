...
float calculatePCFShadowFactor(vec3 pos, vec3 eyePos, vec3 lookAt, mat4 shadowM, sampler2D shadowMap) {
	float shadowFactor = 1.0;
	...
	for (float i = -1.5; i <= 1.5; i = i + 1.0) {
		for (float j = -1.5; j <= 1.5; j = j + 1.0) {
			...
			float shadowDepth = texture(shadowMap, newLightSpacePos).z;
			if (shadowDepth < lightSpacePos.z) {
				// In shadow
				singleShadowFactor = singleShadowFactor + 1.0;
			}				
...
void main() {
	...
	float shadow_factor = calculatePCFShadowFactor(vs_Vertex.xyz, glb_unif_EyePos, glb_unif_LookAt, glb_unif_ShadowM, glb_unif_ShadowMap);
