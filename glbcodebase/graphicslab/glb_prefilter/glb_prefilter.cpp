...
// Date: 2018 / 06/ 03
// Brief: Cube Map Prefilter tool
//----------------------------------------------------------------------
#include "glb_prefilter.h"
#include "resource.h"

#include <iostream>

static const int32_t kPathSize = 128;
static char sEnvMapPath[kPathSize];

class ApplicationPrefilter : public glb::app::ApplicationBase {
public:
    ApplicationPrefilter()
    : m_CubeMapRT(NULL)
    , m_CubeMap(NULL)
    ...
    virtual~ApplicationPrefilter() {}

public:
    static glb::app::ApplicationBase* Create() {
        return new ApplicationPrefilter;
    }
...
public:
    bool Initialize() {
        ...
        // Create texture
        m_ERMap = render::texture::Texture::Create(sEnvMapPath);
    ...
    void Update(float dt) {
        ...
        if (!isIrradianceMapGenerated) {
            ...
            MessageBox(NULL, L"Prefilter OK", L"Info", MB_OK);
        }
...
int _stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPreInstance, LPSTR cmdLine, int nShowCmd) {
    ...
    OPENFILENAME ofn = { 0 };
    TCHAR filePath[kPathSize] = { 0 };
    ofn.lStructSize = sizeof(OPENFILENAME);
    ofn.hwndOwner = NULL;
    ofn.lpstrFilter = L"HDR File(.hdr)\0*.hdr\0All Files(*.*)\0*.*\0\0";
    ofn.nFilterIndex = 1;
    ...
    if (!GetOpenFileName(&ofn)) {  // This method will change current directory
        MessageBox(NULL, L"Please choose a .hdr file", L"Error", MB_OK);
        return -1;
    }

    SetCurrentDirectory(curDirectory);

    wcstombs(sEnvMapPath, filePath, sizeof(sEnvMapPath));

    glb::app::AppConfig config;
    memcpy(config.caption, L"glb_prefilter", sizeof(L"glb_prefilter"));
    config.screen_width = 800;
    config.screen_height = 600;
    config.shadow_map_width = 1024;
    config.shadow_map_height = 1024;
    config.icon = IDI_ICON1;
    if (!glb::app::Application::Initialize(ApplicationPrefilter::Create, hInstance, config)) {
        return 0;
    }
