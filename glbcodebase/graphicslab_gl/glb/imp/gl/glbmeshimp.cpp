...
uint32_t TriangleMesh::Imp::GetVAO() {
    uint32_t ao = -1;

    if (m_VertexBuffer) {
        ao = m_VertexBuffer->GetVAO();
    } else {
        GLB_SAFE_ASSERT(false);
    }
...
uint32_t TriangleMesh::Imp::GetVBO() {
    uint32_t bo = -1;

    if (m_VertexBuffer) {
        bo = m_VertexBuffer->GetVBO();
    } else {
        GLB_SAFE_ASSERT(false);
    }
